...
class sscanPositioner(Device):
    ...
    def __init__(self, prefix: str, num: int, **kwargs: Any) -> None:
        self._ch_num = num
...
def _sscan_positioners(channel_list: List[str]) -> Dict[str, tuple]:
    """Create positioner definitions."""
    defn = OrderedDict()
...
def _sscan_detectors(channel_list: List[str]) -> Dict[str, tuple]:
    """Create detector definitions."""
    defn = OrderedDict()
    for chan in channel_list:
        attr = "d{}".format(chan)
...
def _sscan_triggers(channel_list: List[str]) -> Dict[str, tuple]:
...
class SscanRecord(Device):
    ...
    def set(self, value: Any, **kwargs: Any) -> Optional[DeviceStatus]:
    ...
    def reset(self) -> None:
        ...
        self.number_points.put(0)
        self.maximum_number_points.put(0)
        self.current_point.put(0)
        self.pasm.put("")
        self.execute_scan.put(0)
        self.bspv.put("")
        self.bscd.put(0)
        self.bswait.put(0)
        self.cmnd.put("")
        ...
        for p in self.positioners.component_names:
            getattr(self.positioners, p).reset()
        for d in self.detectors.component_names:
            getattr(self.detectors, d).reset()
        for t in self.triggers.component_names:
            getattr(self.triggers, t).reset()

    def select_channels(self) -> None:
        """Select only the channels configured in EPICS."""
        for p in self.positioners.component_names:
            if not getattr(self.positioners, p).defined_in_EPICS:
                delattr(self.positioners, p)
        for d in self.detectors.component_names:
            if not getattr(self.detectors, d).defined_in_EPICS:
                delattr(self.detectors, d)
        for t in self.triggers.component_names:
            if not getattr(self.triggers, t).defined_in_EPICS:
                delattr(self.triggers, t)

    @property
    def defined_in_EPICS(self) -> bool:
        """True if defined in EPICS."""
        return len(self.desc.get().strip()) > 0
...
class SscanDevice(Device):
    ...
    def reset(self) -> None:
        """Set all fields to default values."""
        self.scan_pause.put(0)
        self.abort_scans.put(0)
        self.resume_delay.put(0)
    ...
    def select_channels(self) -> None:
        ...
        for s in self.component_names:
            if s.startswith("scan"):
                getattr(self, s).select_channels()
