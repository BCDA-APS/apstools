...
- Create and work on a feature branch for every code change unless the user explicitly directs you to skip branching.
- Always start the branch name with the issue number, followed by a concise version of the issue title.
...
- Keep each entry to a concise one-line summary that still respects the repository line-length limit.
